...
## Colab / 구글 드라이브에서 학습하기

로컬 GPU가 없으면 Colab 을 쓴다. 링크 상태 그대로는 업로드가 안 되므로, **실제 파일**로 된 패키지를 하나 만든다.

### 1) 맥/PC — 업로드용 패키지 만들기
```bash
python car_seg_data_split.py --out-dir car_seg_split_upload --image-format jpg --link-mode hardlink
tar -cf car_seg_split_upload.tar car_seg_split_upload      # -h 불필요 — 이미지·마스크 모두 실제 파일
```
- `--image-format jpg`: 사진을 JPG(품질 95)로 변환한 실제 파일로 저장. 원본 대비 **용량 약 1/4**, 화질·학습
  성능 손실은 실측상 무시할 수준(픽셀 MAE 0.7/255, 5-epoch 학습 비교에서 mAP 차이 없음).
- `--link-mode hardlink`: 마스크(`masks/`)도 실제 파일로 저장. **정답 라벨은 항상 PNG 그대로** — JPG는 손실
  압축이라 클래스 경계 픽셀값이 뭉개지므로 라벨에는 절대 쓰지 않는다.
- 이 조합이면 결과 폴더에 링크가 하나도 없어서 `tar -h`(링크 따라가기) 없이 그냥 `tar` 로 묶어도 된다.
- 전체 데이터 기준 tar 크기는 약 14GB (원본 60GB 대비).

### 2) 구글 드라이브에 업로드
만든 `car_seg_split_upload.tar` 를 드라이브에 올린다. 파일 하나로 묶여 있어서 이미지 수만 개를
개별 업로드하는 것보다 훨씬 안정적이다.

### 3) Colab — 마운트 → 로컬 디스크로 압축 해제 → 학습
```python
# 런타임 유형: GPU 로 설정 후 실행
from google.colab import drive
drive.mount('/content/drive')

!tar -xf "/content/drive/MyDrive/<경로>/car_seg_split_upload.tar" -C /content
!sed -i 's|^path:.*|path: /content/car_seg_split_upload|' /content/car_seg_split_upload/data.yaml
!pip install -q ultralytics

!yolo segment train data=/content/car_seg_split_upload/data.yaml model=yolo26s-seg.pt \
    imgsz=1280 epochs=100 batch=-1 device=0 patience=20 \
    project=/content/drive/MyDrive/<경로>/runs name=yolo26s_1280
```
- 드라이브의 tar 를 **코랩 로컬 디스크(`/content`)로 풀고 나서** 학습한다 — 드라이브에 마운트된 채로 직접
  학습하면 이미지 수만 장을 하나씩 읽어야 해서 매우 느리다.
- 결과(`project=`)는 드라이브 경로에 저장한다 — 무료 Colab 은 세션이 끊길 수 있는데, 드라이브에 저장해 두면
  다음 세션에서 `resume` 으로 이어서 학습할 수 있다.
  ```python
  !yolo segment train resume model=/content/drive/MyDrive/<경로>/runs/yolo26s_1280/weights/last.pt
  ```
- 세션이 끊기면 코랩 로컬 디스크는 초기화되므로, 위 `tar -xf` 부터 다시 실행한다.

---

## 로컬 학습 예시 (YOLO)

로컬에 GPU(NVIDIA)가 있거나, Mac 에서 코드가 도는지 짧게 시험할 때. Colab 은 위 섹션 참고.
...
| 분할 | `--image-format jpg` | 사진을 JPG(품질 95)로 변환한 실제 파일로 저장 (마스크는 항상 PNG) |
