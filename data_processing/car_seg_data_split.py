...
출력 (기본: car_seg_split/)
    images/{train,val,test}/        모든 형식이 공유하는 이미지 (원본으로의 상대 링크, --link-mode 로 변경)
                                     --image-format jpg 면 실제 JPG 파일로 변환해서 저장 (용량 약 1/4,
                                     구글드라이브·Colab 업로드용 — 마스크(정답 라벨)는 항상 PNG 그대로 유지)
...
사용 예
    ...
    python car_seg_data_split.py --out-dir car_seg_split_jpg --image-format jpg       # 구글드라이브/Colab 업로드용
...
def _convert_to_jpg(job: tuple[str, str, int]) -> tuple[int, int]:
    """입력 이미지(사진)를 JPG 로 변환. (원본 바이트 수, 변환 후 바이트 수) 반환.

    실측(픽셀 MAE 0.7/255, PSNR 46.9dB, 5-epoch 학습 mAP 비교)상 품질 95 는 화질·성능 손실이
    거의 없다 — 단, 이 함수는 '사진'에만 쓴다. 마스크(정답 라벨)에 JPG 를 쓰면 클래스 경계 픽셀값이
    뭉개져서(실측 8.7%) 안 되므로, 마스크는 항상 place() 로 PNG 그대로 배치한다.
    """
    src, dst, quality = job
...
def split_coco(coco_all: Path, out_dir: Path, df: pd.DataFrame, rename: dict[str, str] | None = None) -> dict:
    """instances_all.json → instances_{train,val,test}.json. image/annotation id 는 원본 그대로 유지.

    df["file_name"] 은 원본(car_seg_dataset) 파일명이어야 한다 (JPG 변환 전에 호출). rename 이 있으면
    (--image-format jpg) 출력 JSON 의 file_name 만 실제 저장된 이름(.jpg)으로 바꿔 쓴다.
    """
    ...
    for s in SPLITS:
        images = [
            {**im, "file_name": rename.get(im["file_name"], im["file_name"])}
            for im in coco["images"] if split_of.get(im["file_name"]) == s
        ]
...
def write_readme(out_dir: Path, formats: list[str], sizes: dict, classes: dict[int, str],
                 coco_stats: dict, image_format: str = "keep") -> None:
    names = [classes[k] for k in sorted(classes)]
    rows = "\n".join(f"| {s} | {v['clips']} | {v['images']:,} |" for s, v in sizes.items())
    images_line = ("← 모든 형식이 공유하는 실제 JPG 파일 (품질 95 — 원본 대비 화질 손실 거의 없음, 용량 약 1/4)"
                   if image_format == "jpg" else
                   "← 모든 형식이 공유하는 이미지 (car_seg_dataset/images 로의 링크)")
...
```
car_seg_split/
├── images/{{train,val,test}}/          {images_line}
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.image_format == "jpg":
        print(f"[split] 이미지를 JPG(품질 {args.jpg_quality})로 변환해 저장 — 링크가 아닌 실제 파일 "
              f"(구글드라이브·Colab 업로드용). 마스크는 항상 PNG 그대로 유지.")
    ...
    missing_files = []
    convert_jobs: list[tuple[str, str, int]] = []
    rename_map: dict[str, str] = {}  # 원본 파일명(car_seg_dataset) → 실제 저장 파일명(car_seg_split) — JPG 변환 시 확장자만 다름
    for row in tqdm(df.itertuples(index=False), total=len(df), desc="파일 배치", unit="img", dynamic_ncols=True):
        ...
        if not src_img.exists() or not src_lbl.exists() or ("mask" in formats and not src_mask.exists()):
            ...
            continue
        if args.image_format == "jpg":
            out_name = f"{Path(row.file_name).stem}.jpg"
            convert_jobs.append((str(src_img), str(out_dir / "images" / row.split / out_name), args.jpg_quality))
        else:
            out_name = row.file_name
            place(src_img, out_dir / "images" / row.split / out_name, args.link_mode)
        rename_map[row.file_name] = out_name
        shutil.copy2(src_lbl, out_dir / "labels" / row.split / src_lbl.name)
        if "mask" in formats:
            place(src_mask, out_dir / "masks" / row.split / src_mask.name, args.link_mode)  # 마스크는 항상 PNG

    image_size_stats = None
    if convert_jobs:
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            sizes = list(tqdm(ex.map(_convert_to_jpg, convert_jobs, chunksize=32), total=len(convert_jobs),
                              desc="JPG 변환", unit="img", dynamic_ncols=True))
        src_total, dst_total = sum(s for s, _ in sizes), sum(d for _, d in sizes)
        image_size_stats = {"source_bytes": src_total, "output_bytes": dst_total}
        print(f"[split] 이미지 용량: {src_total / 1e9:.1f}GB → {dst_total / 1e9:.1f}GB "
              f"({dst_total * 100 / src_total:.0f}%)")
    ...
    coco_stats = {}
    if "coco" in formats:
        # df["file_name"] 이 아직 원본(car_seg_dataset) 이름일 때 호출해야 COCO JSON 과 매칭된다
        coco_stats = split_coco(dataset_dir / "annotations" / "instances_all.json", out_dir, df, rename_map)
    if (dataset_dir / "classes.json").exists():
        shutil.copy2(dataset_dir / "classes.json", out_dir / "classes.json")

    # COCO 분할이 끝난 뒤, df 의 file_name 을 실제 car_seg_split 안의 파일명(JPG 변환 시 .jpg)으로 갱신
    # → 아래 split_manifest.csv · format_preview.png 가 실제 존재하는 파일을 가리키게 된다
    df["file_name"] = df["file_name"].map(rename_map).fillna(df["file_name"])
    ...
    report = {
        ...
        "image_size_bytes": image_size_stats,
    ...
    write_readme(out_dir, formats, sizes, classes, coco_stats, args.image_format)
    if not args.no_plot:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--link-mode", choices=("symlink", "hardlink", "copy"), default="symlink",
                   help="이미지 배치 방식 (symlink: 추가 용량 거의 없음). --image-format jpg 일 때는 무시됨(항상 실제 파일 생성)")
    p.add_argument("--image-format", choices=("keep", "jpg"), default="keep",
                   help="keep: 원본 그대로 링크. jpg: JPG(품질 95)로 변환한 실제 파일로 저장 — 용량 약 1/4, "
                        "구글드라이브·Colab 업로드용. 마스크(정답 라벨)는 항상 PNG 로 유지되어 영향 없음")
    p.add_argument("--jpg-quality", type=int, default=95, metavar="1-100",
                   help="--image-format jpg 일 때의 JPEG 품질")
